...
export default {
    ...
    methods: {
        ...
        async updateTransaction(flag, currency, balance, quoteAsset, baseAsset, quoteCurrency, baseCurrency) {
            console.log({
                    "currency": currency,
                    "quote": quoteAsset,
                    "base": baseAsset
                ...
                await this.user.addUserTransaction(this.userId, flag, currency, Number(quoteAsset), Number(baseAsset))
        ...
        async commitTransaction(currency, quoteAsset, baseAsset, flag) {
            ...
            if (balance[quoteCurrency]["amount"] >= quoteAsset) {
                this.updateTransaction(flag, currency, balance, quoteAsset, baseAsset, quoteCurrency, baseCurrency)
            }
            else {
...
<template>
    <div id="transactor">
        ...
        <br>
        <br>
        <p>{{ transactionFlag }}: {{ transaction.quoteAsset }} {{ transactionFlag == "Buy" ?
                transaction.currency.split("-")[1] : transaction.currency.split("-")[0]
        }}</p>
        <input type="number" v-model.number="transaction.quoteAsset"
            @change="PreviewCoin(this.transactionFlag, this.transaction.currency, this.transaction.quoteAsset)" />
        <br>
        <br>
        <p>Got: {{ transaction.baseAsset }} {{ transactionFlag == "Buy" ? transaction.currency.split("-")[0] :
                transaction.currency.split("-")[1]
        }}</p>
        <br>
        <button class="btn btn-outline-dark"
            @click="commitTransaction(this.transaction.currency, this.transaction.quoteAsset, this.transaction.baseAsset, this.transactionFlag)">Confirm</button>
    </div>
